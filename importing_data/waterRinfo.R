...
L10_077_Tw <- get_timeseries_tsid(
  "39305042",
  from = "2019-01-01",
  to = "2021-02-28"
) #resolution = 15 minutes

g <- ggplot(L10_077_Tw, aes(Timestamp, Value)) +
  geom_line()
write.csv(L10_077_Tw, './data/raw/L10_077_Tw.csv')
...
# OF
# get timeseriesgroup_id from "Gebruik webservices achter waterinfo"
#get right timeseries ID: https://download.waterinfo.be/tsmdownload/KiWIS/KiWIS?datasource=1&service=kisters&type=queryServices&request=gettimeseriesList&timeseriesgroup_id=192901
...
###########################################################################
# velocity (rest of the V data is in cross_sections/get_velocities.R)
ts_id <- 38397042
name <- "L10_077"
L10_077_V <- get_timeseries_tsid(
  ts_id,
  from = "2019-01-01 UTC",
  to = "2021-02-28 UTC"
)
path <- paste('./data/raw/velocity/', name, '_V.csv', sep = "")
write.csv(L10_077_V, path)
