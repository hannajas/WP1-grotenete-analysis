...
source("C:/Code/WP1-grotenete-analysis/src/pdf_trajectory.R")
############################################################################################################
...
g <- g +
  geom_hline(
    yintercept = -1 * data$distance_to_source_m,
    colour = "gray",
    linewidth = 0.5,
    linetype = "dashed"
  )
...
for (i in 1:length(mydfnew.split.eel)) {
  mydfnew.temp <- mydfnew.split.eel[[i]] #for loop wordt doorlopen voor elke i transmitter
  mydfnew.temp$date <- round_date(
    mydfnew.temp$arrival + (mydfnew.temp$departure - mydfnew.temp$arrival) / 2,
    unit = "day"
  )
  ...
  g <- g +
    geom_point(
      aes(arrival, -1 * distance_to_source_m, colour = downstream_migration),
      data = mydfnew.temp,
      shape = 16,
      size = 5
    )
  g <- g + scale_color_manual(values = c("FALSE" = "red", "TRUE" = "green"))
  ...
  g <- g +
    labs(
      ...
      subtitle = mydfnew.temp$catch_year
    )
  g <- g + ylab("Distance (m)")
  g <- g + xlab("Date")
...
for (i in 1:length(mydfnew.split.eel)) {
  ...
  g <- g +
    geom_point(
      aes(arrival, -1 * distance_to_source_m, colour = migration),
      data = mydfnew.temp,
      shape = 16,
      size = 5
    )
...
for (i in 1:length(mydfnew.split.eel)) {
  ...
  g <- g +
    labs(
      title = mydfnew.temp$tag_serial_number[min_date],
      subtitle = mydfnew.temp$catch_year
    )
  g <- g + ylab("Distance (m)")
  g <- g + xlab("Date")
  ...
  g <- g +
    annotate(
      "text",
      x = mydfnew.temp$arrival[1] - (240 * 60 * 60),
      y = -1 * mydfnew.temp$distance_to_source_m,
      label = mydfnew.temp$station_name,
      hjust = 0,
      colour = "red",
...
for (i in 1:length(mydfnew.split.eel)) {
  ...
  g2 <- g2 +
    geom_col(aes(date, R), data = mydfnew.temp, colour = "black", width = 0.01)
  g2 <- g2 +
    geom_point(aes(date, R), data = mydfnew.temp, colour = "black", size = 1)
...
L10_077_Q <- read_csv(
  './data/raw/discharge/L10_077_Q.csv',
  show_col_types = FALSE
) %>%
  rename(
    date = Timestamp
...
cord.dec <- SpatialPoints(
  cbind(data$deploy_latitude, data$deploy_longitude),
...
data$middledate <- as.POSIXct(round_date(
  data$arrival + data$residence / 2,
  "15 mins"
...
for (a in 1:length(traj)) {
  if (nrow(traj[[a]]) > 1) {
    ...
    g <- ggplot(temp_traj) +
      ...
      scale_y_continuous(
        sec.axis = sec_axis(~ . / 22222, name = "Discharge (m³/s)")
      ) +
      theme(
...
P10_011_R <- read_csv(
  './data/raw/rainfall/P10_011_R.csv',
  show_col_types = FALSE
) %>%
  rename(
    date = Timestamp
...
for (a in 1:length(traj)) {
  if (nrow(traj[[a]]) > 1) {
    ...
    g <- ggplot(temp_traj) +
      ...
      geom_line(aes(date, cum_dist), linewidth = 1) +
      geom_line(
        aes(date, cumsum(Value) * 1111),
        colour = "blue",
        linewidth = 1
      ) +
      scale_y_continuous(
        sec.axis = sec_axis(~ . / 1111, name = "Precipitation (mm)")
      ) +
      theme(
        plot.title = element_text(lineheight = .8, face = "bold", size = 20)
      ) +
...
for (a in 1:length(traj)) {
  if (nrow(traj[[a]]) > 1) {
    ...
    g <- ggplot(temp_traj) +
      ...
      scale_y_continuous(
        sec.axis = sec_axis(~ . / 5555, name = "Temperature (°C)")
      ) +
      theme(
        plot.title = element_text(lineheight = .8, face = "bold", size = 20)
      ) +
...
rup02e_SF_1066_turb <- read_csv(
  './data/raw/turbidity/rup02e_SF_1066_turb.csv',
  show_col_types = FALSE
) %>%
  rename(
    date = Timestamp
...
for (a in 1:length(traj)) {
  if (nrow(traj[[a]]) > 1) {
    ...
    g <- ggplot(temp_traj) +
      ...
      scale_y_continuous(
        sec.axis = sec_axis(~ . / 555, name = "Turbidity (NTU)")
      ) +
      theme(
        plot.title = element_text(lineheight = .8, face = "bold", size = 20)
      ) +
...
rup02e_SF_1066_O <- read_csv(
  './data/raw/oxygen/rup02e_SF_1066_O.csv',
  show_col_types = FALSE
) %>%
  rename(
    date = Timestamp
...
for (a in 1:length(traj)) {
  if (nrow(traj[[a]]) > 1) {
    ...
    g <- ggplot(temp_traj) +
      ...
      geom_line(aes(date, Value * 5555), colour = "blue", linewidth = 1) +
      scale_y_continuous(
        sec.axis = sec_axis(~ . / 5555, name = "Turbidity (NTU)")
      ) +
      theme(
        plot.title = element_text(lineheight = .8, face = "bold", size = 20)
      ) +
...
rup02e_SF_1066_O <- read_csv(
  './data/interim/processed/rup02e_SF_1066_O.csv',
  show_col_types = FALSE
) %>%
  rename(
    date = Timestamp
...
rup02e_SF_1066_S <- read_csv(
  './data/interim/processed/rup02e_SF_1066_S.csv',
  show_col_types = FALSE
) %>%
  rename(
    date = Timestamp
...
for (a in 1:length(traj)) {
  if (nrow(traj[[a]]) > 1) {
    ...
    g1 <- ggplot(temp_traj_1) +
      ...
      scale_y_continuous(
        sec.axis = sec_axis(~ . / 300, name = "Turbidity (NTU)")
      ) +
      theme(
        plot.title = element_text(lineheight = .8, face = "bold", size = 20)
      ) +
    ...
    g2 <- ggplot(temp_traj_2) +
      ...
      scale_y_continuous(
        sec.axis = sec_axis(~ . / 4000, name = "Oxygen (mg/l)")
      ) +
      theme(
        plot.title = element_text(lineheight = .8, face = "bold", size = 20)
      ) +
    ...
    g3 <- ggplot(temp_traj_3) +
      ...
      scale_y_continuous(
        sec.axis = sec_axis(~ . / 40000, name = "Salinity (spu)")
      ) +
      theme(
        plot.title = element_text(lineheight = .8, face = "bold", size = 20)
      ) +
...
for (i in 1:length(mydfnew.split.eel)) {
  ...
  env_data <- rup00a_1066_Q[
    rup00a_1066_Q$date >= min & rup00a_1066_Q$date <= max,
  ]
  g2 <- ggplot() +
    geom_point(
      aes(date, Value),
...
#O
pdf(
  "C:/Code/WP1-grotenete-analysis/figures/Trajectory_linked_env_var/traj_O.pdf"
)
mydfnew.split.eel <- split(data, data$tag_serial_number) # split dataset based on tag IDs
...
###########################################################################################################
pdf(
  "C:/Code/WP1-grotenete-analysis/figures/Trajectory_linked_env_var/traj_inter_Q.pdf"
) # Create pdf
mydfnew.split.eel <- split(data_inter_env, data_inter_env$tag_serial_number)
for (eel in names(mydfnew.split.eel)) {
  mydfnew <- mydfnew.split.eel[[eel]]
  mydfnew$cluster <- as.factor(mydfnew$cluster)
  # only if more than 1 row
  if (!("Q" %in% names(mydfnew)) || all(is.na(mydfnew$Q))) {
    next
  }
  plots <- plot_one_traj(
    mydfnew,
