...
#save inter_all on computer
#saveRDS(inter_all, "./data/interim/interpolated_data_5_min.rds")
...
data_cluster <- left_join(
  data,
  data_inter[, c("id", "date", "cluster", "is_original")],
...
# plot clusters
data_filter <- data %>%
  filter(tag_serial_number == "1294161" & !is.na(speed_m_s))
...
library(factoextra)
speed <- data %>%
  filter(!is.na(speed_m_s) & tag_serial_number == "1294161") %>%
