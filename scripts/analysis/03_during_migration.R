...
########################################################
# General durations of resting pauzes during migration
########################################################
#duration of the label resting with NO 'migratory' in between
resting_durations <- data %>%
  group_by(tag_serial_number) %>%
  mutate(
    state_change = (label != lag(label)) | (is.na(lag(label))),
    bout_id = cumsum(replace_na(state_change, 0)),
    pref_departure = lag(departure)
  ) %>%
  ungroup() %>%
  filter(label == "resting" & bout_id > 1) %>%
  group_by(tag_serial_number, bout_id) %>%
  summarize(
    start_time = min(pref_departure, na.rm = TRUE),
    end_time = max(departure, na.rm = TRUE),
    duration = as.numeric(difftime(end_time, start_time, units = "hours"))
  ) %>%
  ungroup()

#filter durations > 1 week
resting_durations <- resting_durations %>%
  filter(duration >= 168) #ook bij 1293822 dus iets misgelopen
