...
result <- kmeans(log_speed[not_na_idx], centers = k, nstart = 20, iter.max = 100)
...
#density plot
ggplot(data[not_na_idx,], aes(x = speed_m_s, color = cluster)) +
...
ggplot(data[not_na_idx,], aes(x = log(speed_m_s), y = 0, color = cluster)) +
...
fviz_nbclust(as.matrix(log(data$speed_m_s[not_na_idx])), kmeans, method = "silhouette")
#2 is het optimale aantal clusters!


#calculated border between clusters
border <- exp(mean(result$centers)) #m/s