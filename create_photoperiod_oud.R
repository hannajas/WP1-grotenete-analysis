...
for (i in 1:length(mydfnew.split.eel)){
    ...
    mydfnew.temp <- full_join(env_data, mydfnew.temp, by = "date")

    #range of times of detection
    min_date <- which(min(mydfnew.temp$arrival, na.rm=TRUE)==mydfnew.temp$arrival)
    max_date <- which(max(mydfnew.temp$arrival, na.rm=TRUE)==mydfnew.temp$arrival)
    ...
    mydfenv.temp <- mydfnew.temp[min_date:max_date,1:ncol(mydfnew.temp)]
    g2 <- ggplot()
